...
#pragma region window and opengl
  ...
  permaAssertComment(gladLoadGLLoader((GLADloadproc)glfwGetProcAddress),
                     "err initializing glad");
...
#pragma region deltaTime
    ...
    float deltaTime =
        (std::chrono::duration_cast<std::chrono::nanoseconds>(start - stop))
            .count() /
        1000000000.0;
...
#pragma region fullscreen
    ...
    if (platform::isFocused() && currentFullScreen != fullScreen) {
      ...
      if (fullScreen) {
        ...
        // glfwWindowHint(GLFW_DECORATED, NULL); // Remove the border and
        // titlebar..
        ...
        glfwSetWindowMonitor(wind, monitor, 0, 0, mode->width, mode->height,
                             mode->refreshRate);
        ...
        glfwSetWindowMonitor(wind, nullptr, lastPosX, lastPosY, lastW, lastH,
                             0);
